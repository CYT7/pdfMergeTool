...
class UiWindows(object):
    ...
    def reset_ui(self, main_windows):
        main_windows.setWindowTitle("主窗口")
        self.label.setText("输出文件路径")
        self.select_button.setText("浏览")
        self.group_box.setTitle("合并文件列表")
        self.add_button.setText("添加")
        self.delete_button.setText("删除")
        self.up_button.setText("上移")
        self.down_button.setText("下移")
        self.clear_button.setText("清空")
        self.action_button.setText("执行")
    ...
    def generatePdf(self):
        ...
        if not output_file:
            QMessageBox.warning(None, '提示', '请输入输出路径！')
            return
        merge_files = [self.list_control_file.item(index).text() for index in range(self.list_control_file.count())]
        if not merge_files:
            QMessageBox.warning(None, '提示', '请选择需要合并的pdf文件！')
            return
        file_writer = PyPDF2.PdfFileWriter()
        for merge_file in merge_files:
            merge_page(file_writer, merge_file)
        file_writer.write(open(output_file, 'wb'))
        end_time = datetime.datetime.now()  # 结束时间
        print('合并时间=', (end_time - start_time).seconds, 's')
        QMessageBox.information(None, '提示', '合并完成！')


if __name__ == '__main__':
    app = QApplication()
    main_window = QMainWindow()
    ui = UiWindows(main_window)
